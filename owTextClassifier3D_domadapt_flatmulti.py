...
parser.add_argument('--place', action='store_true',
                    help='apply domain adapatation for Google places')
...
def load_embeddings(nb_words=None, emb_dim=200, index_from=3,
    ...
    return embedding_matrix, feat_sizes
...
class CNN(nn.Module):
    ...
    def forward(self, inputs, test_mode=False, domain=[]):
...
def predict(net, x, f, batch_size, intermediate=False, domain=[]):
    pred = np.empty(0)
    ...
    for b in range(batches):
        ...
        # Only one domain
            ...
            for i in range(0, len(bx)):
                case_x = bx[i]
                case_x = case_x.view(-1,1)
                case_f = bf[i]
                
                case_d = list()
                for d in domain:
                    case_d.append(case_f[d])
                    
                case_p = net(case_x,domain=case_d)
                y_pred_list.append(case_p)
              
            b_pred = torch.cat(y_pred_list)
...
def train(net, x, y, f, nepochs, batch_size, domain=[]):
...
emb_dim = 200
embeddings, feats_size = load_embeddings(nb_words=max_features, emb_dim=emb_dim)
...
domain = list()
didx = 0
for f in range(0,len(feats_size)):
    fsize = feats_size[f]
    if f in domain_opt:
        num_feats += fsize
