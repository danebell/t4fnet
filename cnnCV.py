...
def pad3d(sequences, maxtweets=None, maxlen=None, dtype='int32',
          padding='pre', truncating='pre', value=0., start=0):
    '''
        # Returns
    ...
    for idx, s in enumerate(sequences):
        if len(s) <= start:
            continue # no tweets
        x[idx, :min(mt+start,len(s)-start)] = sequence.pad_sequences(s[start:(mt+start)], ml, dtype, padding, truncating, value)
    return x
...
cnnw = np.zeros(len(y_pos) + len(y_neg))
lstm = np.zeros(len(y_pos) + len(y_neg))
